...
- [Cilium](#cilium)
  ...
  - [3. eBPF를 이용한 Packet Flow (eBPF Datapath)](#3-ebpf를-이용한-packet-flow-ebpf-datapath)
    - [같은 노드에 배치된 Pod 간의 통신 경로](#같은-노드에-배치된-pod-간의-통신-경로)
    - [Pod에서 외부로 나가는 트래픽 통신 경로](#pod에서-외부로-나가는-트래픽-통신-경로)
    - [Pod 내부로 들어오는 트래픽 통신 경로](#pod-내부로-들어오는-트래픽-통신-경로)
...
#### 같은 노드에 배치된 Pod 간의 통신 경로

<p align><img src="./_image/packet_flow_endpoint_to_endpoint.png" title="출처: Cilium Documentation - Networking.eBPF_Datapath.Life_of_a_Packet" width="60%"></p>
